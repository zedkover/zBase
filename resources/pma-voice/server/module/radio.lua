...
function removePlayerFromRadio(source, radioChannel)
	print('[radio] Removed %s from radio %s', source, radioChannel)
...
function addChannelCheck(channel, cb)
	...
	print("%s added a check to channel %s", GetInvokingResource(), channel)
end
...
function setTalkingOnRadio(talking)
	...
	if radioTbl then
		print('[radio] Set %s to talking: %s on radio %s',source, talking, plyVoice.radio)
