	...
	for i = 1, #targets do
		for id, _ in pairs(targets[i]) do
			...
			if not addedPlayers[id] then
				...
				MumbleSetVolumeOverrideByServerId(id, volumes['radio'])
			end
			::skip_loop::
		end
	end
...
function NotificationVoice(msg)
...
RegisterCommand('+intensitevoix', function()
	...
	-- NotificationVoice('L\'intensité de votre voix a été définit sur '..Cfg.voiceModes[voiceMode][2])

	NotificationVoice('~w~Nouvelle intensité définie:\n'..'~'..Cfg.voiceModes[voiceMode][3]..'~'..Cfg.voiceModes[voiceMode][2].."~s~")
...
RegisterKeyMapping('+intensitevoix', 'Changez la distance de la voix', 'keyboard', GetConvar('voice_defaultCycle', 'F1'))
...
RegisterNetEvent('zRadio:talkRadio')
AddEventHandler('zRadio:talkRadio', function(status)
	radioActive = status
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if GetConvarInt('voice_enableUi', 1) == 1 then
			if lastRadioStatus ~= radioActive or lastTalkingStatus ~= (NetworkIsPlayerTalking(PlayerId()) == 1) then
				lastRadioStatus = radioActive and NetworkIsPlayerTalking(PlayerId()) == 1
				lastTalkingStatus = NetworkIsPlayerTalking(PlayerId()) == 1
				
				if lastTalkingStatus == false then
					TriggerEvent('zRadio:setIndicatorMode', 0)
				elseif lastRadioStatus == false then
					ExecuteCommand('-radiotalk')
					TriggerEvent('zRadio:setIndicatorMode', 0)
					TriggerEvent('zRadio:setIndicatorMode', mode)
				elseif lastRadioStatus == true then	
					ExecuteCommand('+radiotalk')
					TriggerEvent('zRadio:setIndicatorMode', 5)
				end
			end
		end
