...
function notifyAlertSMS (number, alert, listSrc)
    if PhoneNumbers[number] ~= nil then
      ...
      for k, _ in pairs(listSrc) do
        getPhoneNumber(tonumber(k), function (n)
          if n ~= nil then
            TriggerEvent('gcPhone:_internalAddMessagedumpamistocazzo', number, n, mess, 0, function (smsMess)
  ...
  AddEventHandler('esx_phone:send', function(number, message, _, coords)
    ...
    if PhoneNumbers[number] ~= nil then
      getPhoneNumber(source, function (phone) 
        notifyAlertSMS(number, {message = message, coords = coords, numero = phone,}, PhoneNumbers[number].sources)
      end)
    end
  end)
  ...
  local item = {
    ...
    {'burgershot', 'alerte burgershot'},
    {'realestateagent', 'alerte dynasty8'},
    {'barber', 'alerte barber'},
    {'tattoo', 'alerte tattoo'},
    {'lscustom', 'alerte lscustom'},
    {'unicorn', 'alerte unicorn'},
    {'ambulance', 'alerte ambulance'}
  }
  
  for _,v in pairs(item) do
      TriggerEvent('esx_phone:registerNumber', v[1], v[2], true, true)
      AddEventHandler('onResourceStop', function(resource)
        if resource == GetCurrentResourceName() then
          TriggerEvent('esx_phone:removeNumber', v[1])
        end
      end)
  end
...
function getPhoneRandomNumber()
    ...
    local num = string.format("555-%04d", numBase1)
