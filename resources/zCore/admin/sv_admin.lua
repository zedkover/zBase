RegisterServerEvent('zCore:PlayerEvent')
AddEventHandler("zCore:PlayerEvent",function(name, source, r, a, b, c)
    TriggerClientEvent(name, source, r, a, b, c)
end)

RegisterServerEvent("zCore:KickPlayer")
AddEventHandler("zCore:KickPlayer", function(player, reason)
...
ESX.RegisterServerCallback('zCore:GetBan', function(source, cb, f)
    MySQL.Async.fetchAll("SELECT * FROM blacklist", {}, function(result)
        cb(result)
    end)
end)

ESX.RegisterServerCallback('zCore:GetGroup', function(source, cb)
...
RegisterServerEvent("zCore:DeleteBan")
AddEventHandler("zCore:DeleteBan", function(banId, name)
...
RegisterServerEvent("zCore:Ban")
AddEventHandler("zCore:Ban", function(player, raison, pName)
    if not raison then
...
RegisterServerEvent('zCore:SearchBanOffline')
AddEventHandler('zCore:SearchBanOffline', function(target)
        ...
        if target ~= "" then
            MySQL.Async.fetchAll('SELECT * FROM baninfo WHERE id = @id',
            {
                ...
                if data[1] then
                    ...
                    TriggerEvent('zCore:BanOff', data[1].identifier or 0, data[1].license or 0, data[1].discord or 0, data[1].playername, reason, pName, data[1].playerip or 0, data[1].xblid or 0, data[1].liveid or 0, data[1].Token or 0)
...
RegisterServerEvent('zCore:BanPlayerOffline')
AddEventHandler('zCore:BanPlayerOffline', function(permId, reason, pName)
    BanPlayerOffline(source, permId, reason, pName)
end)

RegisterServerEvent("zCore:BanOff")
AddEventHandler("zCore:BanOff", function(steam, license, discord, name, raison, pName, ip, xbl, live, token)
...
AddEventHandler('playerConnecting', function(name, skr, d)
    ...
    d.defer()
    d.presentCard([==[{"type":"AdaptiveCard","version":"1.0","body":[{"type":"ColumnSet","columns":[{"type":"Column","width":"auto","items":[{"type":"Image","altText":"","url":"","size":"Medium"}]},{"type":"Column","width":"stretch","items":[{"type":"TextBlock","text":"Admin","weight":"Bolder","size":"Medium"},{"type":"TextBlock","text":"Blacklist"}]}]},{"type":"TextBlock","text":"Initialisation de la connexion au proxy..","size":"Medium","weight":"Lighter"}],"$schema":"http://adaptivecards.io/schemas/adaptive-card.json"}]==], function(data, rawData)
    end)
