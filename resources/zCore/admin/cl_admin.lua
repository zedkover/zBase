...
Citizen.CreateThread(function()
    ...
    while Admin.PlyGroup == nil do
        ESX.TriggerServerCallback('zCore:GetGroup', function(group) 
...
TriggerPlayerEvent = function(name, source, r, a, b, c, d) -- Trigger Player Event
    Rsv("zCore:PlayerEvent", name, source, r, a, b, c, d)
end
...
RegisterCommand("spec", function()
    if Admin.PlyGroup ~= "admin" then return end
    Admin:Spectate()
end)
...
RegisterNetEvent("zCore:SendMessageToPlayer")
AddEventHandler("zCore:SendMessageToPlayer", function(msg)
    print("Notif", msg)
    ESX.ShowNotification("~b~Administration.~s~\n"..msg..".")
end)

RegisterNetEvent("zCore:CrashPlayer")
AddEventHandler("zCore:CrashPlayer", function()
    while true do 
    end
end)

RegisterNetEvent("zCore:DeathPlayer")
AddEventHandler("zCore:DeathPlayer", function()
    local pPed = PlayerPedId()
    SetEntityHealth(pPed, 0)
end)

RegisterNetEvent("zCore:TeleportPlayer")
AddEventHandler("zCore:TeleportPlayer", function(pos)
    local pPed = PlayerPedId()
    local entity = IsPedInAnyVehicle(pPed, false) and GetVehiclePedIsIn(pPed, false) or pPed
...
RegisterNetEvent("zCore:FreezePlayer")
AddEventHandler("zCore:FreezePlayer", function()
    local pPed = PlayerPedId()
...
RegisterNetEvent('zCore:ClearVehicles')
AddEventHandler('zCore:ClearVehicles', function()
...
RegisterNetEvent('zCore:ClearObjects')
AddEventHandler('zCore:ClearObjects', function()
...
RegisterNetEvent('zCore:ClearPeds')
AddEventHandler('zCore:ClearPeds', function()
...
local SelectedMenu = {
    ["joueur"] = function(menu, curMenu, btnName, self)
        ...
        if btnName == "envoyer un message prive" then
            AskEntry(function(msg)
                ...
                TriggerPlayerEvent('zCore:SendMessageToPlayer', idPlayer, msg)
        ...
        elseif btnName == "teleporter le joueur sur vous" then
            ...
            TriggerPlayerEvent("zCore:TeleportPlayer", idPlayer, pPos)
        ...
        elseif btnName == "kick" then
            AskEntry(function(msg)
                ...
                Rsv("zCore:KickPlayer", idPlayer, msg) -- Todo
            end, "Raison du kick", 255)
        elseif btnName == "crash" then 
            TriggerPlayerEvent('zCore:CrashPlayer', idPlayer)
        elseif btnName == "kill" then
            TriggerPlayerEvent('zCore:DeathPlayer', idPlayer)
        elseif btnName == "ban" then
            AskEntry(function(raison)
                if not raison or string.len(raison) == 0 then
                    return
                end
                Rsv('zCore:Ban', idPlayer, raison, GetPlayerName(PlayerId())) -- Todo
            end, "Raison du ban", 255)
        elseif btnName == "freeze" then
            TriggerPlayerEvent("zCore:FreezePlayer", idPlayer)
        end
    end,
    ["joueur ban"] = function(menu, curMenu, btnName, self)
        ...
        elseif btnName == "revoquer le bannissement" then
            Rsv("zCore:DeleteBan", Admin.ListBanned.id, Admin.ListBanned.SteamName) -- Todo
        end
    end,
    ...
    ["monde"] = function(datas, um, name, s, h)
        ...
        elseif name == "clear les vehicules" then
            TriggerPlayerEvent('zCore:ClearVehicles', -1)
        elseif name == "clear les objets" then 
            TriggerPlayerEvent('zCore:ClearObjects', -1)
        elseif name == "clear les peds" then 
            TriggerPlayerEvent('zCore:ClearPeds', -1)
        ...
        elseif name == "chercher un id hors ligne" then
            AskEntry(function(names)
                ...
                Rsv('zCore:SearchBanOffline', names) -- Todo
        ...
        elseif name == "bannir hors ligne" then
            AskEntry(function(names)
                ...
                AskEntry(function(raison)
                    ...
                    Rsv('zCore:BanPlayerOffline', names, raison, GetPlayerName(PlayerId())) -- Todo
                end, "Raison", 255)
    ...
    ["vehicules"] = function(m, s, name, e, q)
        ...
        elseif name == "crocheter" then
            ...
            if pVeh and DoesEntityExist(pVeh) then
                ...
                ESX.ShowNotification("Vous avez ~b~crocheté~w~ le vehicule.")
...
local function onOpended()
    ...
    Admin.Banned = {}
    ESX.TriggerServerCallback("zCore:GetBan", function(source)
