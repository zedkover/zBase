...
ConfzBank              = {}
ConfzBank.DrawDistance = 100
ConfzBank.Size         = {x = 1.0, y = 1.0, z = 1.0}
ConfzBank.Color        = {r = 255, g = 255, b = 255}
ConfzBank.Type         = 29
...
local Banque = {
        Base = { Header = {"commonmenu", "interaction_bgd"}, Color = {color_black}, HeaderColor = {255, 255, 255}, Title = "Banque"},
        ...
        onSelected = function(self, _, btn, CMenu, menuData, currentButton, currentSlt, result)
            ...
            if btn.name == "Retirer" then
                retirer()
                 ...
                 if btn.name == "~b~Regarder votre Solde Bancaire" then
                    actusolde()
                end
        end,
    },
    ...
    Menu = {
        ["Compte Banquaire"] = {
            b = {
                ...
                {name = "Retirer", ask = "→", askX = true},
                {name = "~b~Regarder votre Solde Bancaire", ask = "→", askX = true},
            }
        },
    }
} 
...
function actusolde()
    TriggerServerEvent("zBank:actusold", action)
end

RegisterNetEvent("zBank:actusold")
AddEventHandler("zBank:actusold", function(money, cash)
...
function retirer()
    local amount = KeyboardInput('Retirer de l\'espèces', '', 10)
    ...
    if amount ~= nil then
        ...
        if type(amount) == 'number' then
            TriggerServerEvent('zBank:RetirerArgent', amount)
        end
    end
end
...
function deposer()
    ...
    if amount ~= nil then
        ...
        if type(amount) == 'number' then
            TriggerServerEvent('zBank:DeposerArgent', amount)
        end
    end
end	
