...
RegisterNetEvent("zFuel:refuelVeh")
AddEventHandler("zFuel:refuelVeh",function(v3,ihKb)
	local JGSK=GetClosestVehicle2(GetEntityCoords(GetPlayerPed(-1)),5.0,ihKb)
	if not JGSK or not DoesEntityExist(JGSK)then 
		ESX.ShowNotification("~r~Le véhicule n'a pas été trouvé.")
		return 
	end
...
local function DVs8kf2w(lcBL,DHPxI,dx,RRuSHnxf)
	...
	local Rr,scRP0=math.floor(GetVehicleFuelLevel(mcYOuT)),math.floor(GetVehicleHandlingFloat(mcYOuT,"CHandlingData","fPetrolTankVolume"))
		if RRuSHnxf==1 then 
			...
			notifidfuel = ESX.ShowNotification("~o~Globe Oil\n~s~Réservoir du véhicule: ~b~"..Rr.."L~s~.")
	...
	if AI0R2TQ6 <1 then
		ESX.ShowNotification("~r~Le réservoir est trop remplit. "..scRP0 .."/"..scRP0 .."L")return end
		TriggerServerEvent('fuel:pay', AI0R2TQ6)
		ESX.ShowNotification("~o~Globe Oil\n~s~Vous venez de payez ~b~"..AI0R2TQ6.."$~s~ pour ~b~"..AI0R2TQ6.."L~s~.")
        ESX.ShowNotification("~b~Le réservoir est entrain de se remplir")
		TriggerEvent('zFuel:refuelVeh', AI0R2TQ6, GetEntityModel(mcYOuT))
		lcBL:CloseMenu()
	end 
end
...
local function vms5(yA)
	...
	local M7={Base = { Header = {"shopui_title_gasstation", "shopui_title_gasstation"},HeaderColor = {255,255,255}, world = true},Data={currentMenu="station essence",},
...
function FuelInitialize()
	for oh,DZXGTh in pairs(O3_X)do
		createBlip(DZXGTh,361,1,"Station d'essence",false,0.4)
	end
end
...
function Utils_UserFuelPetrolTank(bJfct,DsXd)
    ...
    if _4O and DoesEntityExist(_4O) and (GetEntityBoneIndexByName(_4O,"petroltank")==-1 and GetDistanceBetweenCoords(Dzg,GetEntityCoords(_4O),true)<2.5 or GetDistanceBetweenCoords(Dzg,GetWorldPositionOfEntityBone(_4O,GetEntityBoneIndexByName(_4O,"petroltank")),true)<2)then
        ...
        SetTimeout(10000,function()
            ...
            ESX.ShowNotification("~b~Vous avez fait le plein du véhicule.")
			...
			TriggerServerEvent("zEssence:removeitem", "essence", 1)
        end)
    end 
end;

RegisterNetEvent("zEssence:usessence")
AddEventHandler("zEssence:usessence", function()
