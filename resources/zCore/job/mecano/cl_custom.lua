...
RegisterNetEvent('zCustom:StartParticles')
AddEventHandler('zCustom:StartParticles', function(pos, effectData, size)
...
local function RenderBackProperties(bool)
	if bool then 
		LsCustoms.Data.props = ESX.Game.GetVehicleProperties(LsCustoms.Data.veh)
	elseif not bool then 
		ESX.Game.SetVehicleProperties(LsCustoms.Data.veh, LsCustoms.Data.props)
	end
end

local slideFunction = {
	["intérieurs"] = function(veh, b)
	...
	["tableau de bord"] = function(veh, b) SetVehicleDashboardColour(veh, b.slidenum and b.slidenum - 1 or 0) end,
...
local function onSlide(menuData, currentButton, currentSlt, self)
	...
	if currentMenu == "primaire" or currentMenu == "secondaires" then -- extra args
...
RegisterNetEvent('zCustom:InstallLsCustoms')
AddEventHandler('zCustom:InstallLsCustoms', function()
	LsCustoms.OnInstall = true
    LsCustoms.Data.props = ESX.Game.GetVehicleProperties(LsCustoms.Data.veh)
	local pPed = player
	print(LsCustoms.Data.props)

	ESX.ShowNotification("Vous avez ~b~installé~s~ toutes ~b~les modifications~s~.")
	LsCustoms.OnInstall = false
	LsCustoms.Data = {}
end)

RegisterNetEvent('zCustom:CancelLsCustoms')
AddEventHandler('zCustom:CancelLsCustoms', function()
	ESX.ShowNotification("La ~r~commande~s~ a été ~r~refusé~s~ car l'entreprise n'a plus assez de ~r~fonds nécessaires~s~.")
	ESX.Game.SetVehicleProperties(LsCustoms.Data.veh, LsCustoms.Data.propsVeh)
end)
...
local function onSelected(PMenu, menuData, currentButton, currentSlt, allButtons)
	...
	if currentMenu == "customisation" then
		if currentButton.name == "~b~Confirmer" and LsCustoms.Data.veh and DoesEntityExist(LsCustoms.Data.veh) then
            TriggerServerEvent("zCustom:BuyLsCustoms", LsCustoms.Price)
			Wait(500)
			PMenu:CloseMenu()
            LsCustoms.Price = 0
		end
		...
		elseif currentMenu == "primaire" or currentMenu == "secondaires" then
			customPr = currentButton.prix
			RenderBackProperties(true)
            BuyCustoms(customPr)
			local boolSecondary = currentMenu == "secondaires"
		elseif currentMenu == "peinture" then
			if buttonName == "intérieurs" then
				RenderBackProperties(true)
                BuyCustoms(currentButton.prix)
			elseif buttonName == "tableau de bord" then
				RenderBackProperties(true)
                BuyCustoms(currentButton.prix)
...
local function onRender(self, allButtons, currentButton)
	...
	if LsCustoms.Price > 0 then 
		DrawTextScreen(0.5, 0.90, 1.1, "Total facture: ~b~"..(LsCustoms.Price*1.6).."$~s~. Total entreprise: ~b~"..LsCustoms.Price.."$~s~.", 4, 0)
	end
...
LsCustoms.LSCustomMenu = {
	...
	Events = {
		onOpened = function()
			...
			LsCustoms.Data.propsVeh = ESX.Game.GetVehicleProperties(LsCustoms.Data.veh)
            LsCustoms.Data.props = ESX.Game.GetVehicleProperties(LsCustoms.Data.veh)
	...
	Menu = {
		...
		["classiques"] = {
			b = {
				...
				{ name = "Livery", prix = 700, slidemax = function() return GetVehicleLiveryCount(LsCustoms.Data.veh) - 1 end },
		...
		["peinture"] = {
			...
			b = {
				{ name = "Primaire" },
				{ name = "Secondaires" },
				{ name = "Intérieurs", slidemax = 158, prix = 125 },
				{ name = "Tableau de bord", slidemax = 158, prix = 125 },
