...
OuvrirKeyboard = function(entryTitle, textEntry, inputText, maxLength)
...
function PMenu:drawMenuButton(button, intX, intY, boolSelected, intW, intH, intID)
	...
	if canUse then
		...
		local textBonus = (self.Events["setBonus"] and self.Events["setBonus"](button, self.Data.currentMenu, self)) or (button.amount and "~b~" .. math.floor(button.amount) .. "$") or (button.price and "~b~" .. button.price .. "$")
...
function setheader(header, var)
	headername = {header = header, var = var}
end
...
function PMenu:DrawHeader(intCount)
	...
	if boolHeader then
		...
		if not self.Base.CustomHeader then
			DrawText2(4, self.Base.Title, .8, self.Width  - spriteW / 2, self.Height - 0.006 - intH / 2 + .0125, color_white, false, 0)
		end
	end
...
function PMenu:DrawHelpers2(tableButtons)
	local hasHelp = self.Base.Arrowsonly or self.Menu[self.Data.currentMenu] and self.Menu[self.Data.currentMenu].Arrowsonly or tableButtons[self.Pag[3]] and tableButtons[self.Pag[3]].Arrowsonly
	if hasHelp then
		...
		DrawSprite("commonmenu", "gradient_bgd", self.Width - _intW / 2, self.Height + nwintH / 2 - 0.020, _intW, 0.035, .0, 255, 255, 255, 255)
		DrawSprite("commonmenu", "shop_arrows_upanddown", self.Width - _intW / 2, self.Height + nwintH / 2 - 0.020, 0.027, 0.04, .0, 255, 255, 255, 255)
		end
	end
end
...
function PMenu:DrawHelpers(tableButtons)
	...
	if hasHelp then
		local intH, scale = 0.029, 0.260
		self.Height = self.Height - 0.040
...
function PMenu:Draw()
	...
	if tableButtons and intCount and not self.Invisible then
		...
		self:DrawHelpers2(tableButtons) -- 0.00ms
