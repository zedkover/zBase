...
RegisterNetEvent("zProperty_chest_inventory:setOwnedVehicule")
AddEventHandler("zProperty_chest_inventory:setOwnedVehicule", function(m)
    e = m
end)
...
function OpenPropertyChest(M, N, myVeh)
    ESX.TriggerServerCallback("zProperty_chest:getInventoryV", function(O)
        ...
        TriggerEvent("zPropertyInv:openChestInventory", data, O.blackMoney, O.items, O.weapons, O.money)
    end, M)
end
function all_trim(P)
    if P then
...
RegisterNetEvent("zPropertyInv:openChestInventory")
AddEventHandler("zPropertyInv:openChestInventory", function(k, M, inventory, weapons, money)
...
RegisterNetEvent("zPropertyInv:refreshChestInventory")
AddEventHandler("zPropertyInv:refreshChestInventory", function(k, M, inventory, weapons, money)
    setChestInventoryData(k, M, inventory, weapons, money)
end)
function setChestInventoryData(k, M, inventory, weapons, money)
    ...
    if not Property.ChestHUD then 
        ...
        Wait(100)
        local menuTemplate = {
            Base = { Title = "Coffre", Header = {"commonmenu", "interaction_bgd"}, world = true },
                ...
                onSelected = function(self, menuData, btnData, currentSlt, allButtons)
                    ...
                    if menuData.currentMenu == "vos objets" then 
                        ...
                        AskEntry(function(msg)
                            ...
                            if count then 
                                ...
                                TriggerServerEvent("zProperty_chest:putItem", d.plate, btnData.type, btnData.item, count, d.max, d.myVeh, btnData.name)
                                Wait(50)
                                LoadInvProperty()
                            else
                    ...
                    elseif menuData.currentMenu == "les objets" then 
                        ...
                        AskEntry(function(msg)
                            local count = tonumber(msg)
                            if count then 
                                TriggerServerEvent("zProperty_chest:getItem", d.plate, btnData.type, btnData.item, count, d.max, d.myVeh)
                                Wait(50)
                                LoadInvProperty()
                            else
                                ESX.ShowNotification("~r~Veuillez entrez une quantité valide.")
            ...
            Menu = {
                ...
                ["vos objets"] = {
                    b = function()
                        local table = {}
                        ESX.TriggerServerCallback("zPropertyInv:getPlayerInventory", function(k)
                            items = {}
                            InvMenu = k.inventory
...
RegisterNUICallback("PutIntoChest", function(k, l)
    ...
    if type(k.number) == "number" and math.floor(k.number) == k.number then
        ...
        TriggerServerEvent("zProperty_chest:putItem", d.plate, k.item.type, k.item.name, s, d.max, d.myVeh, k.item.label)
    end
    Wait(50)
    LoadInvProperty()
    l("ok")
...
RegisterNUICallback("TakeFromChest", function(k, l)
    local m = GetPlayerPed(-1)
    if type(k.number) == "number" and math.floor(k.number) == k.number then
        TriggerServerEvent("zProperty_chest:getItem", d.plate, k.item.type, k.item.name, tonumber(k.number), d.max, d.myVeh)
    end
    Wait(50)
    LoadInvProperty()
    l("ok")
